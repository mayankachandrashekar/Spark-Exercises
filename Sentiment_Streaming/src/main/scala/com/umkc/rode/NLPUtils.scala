...
//    }
  ...
  def tfidfTransformerTest(data: DStream[LabeledDocumentTest], norm: Boolean = false): DStream[Vector] = {
    val Hashingtf = new HashingTF()
    val tf: DStream[Vector] = data.map {
      x =>
        val tff: Vector = Hashingtf.transform(x.body)

        tff
    }
   /* tf.foreachRDD(x=>print(x))
    tf.foreachRDD {
    xrdd =>
      val idfModel = new IDF().fit(xrdd)
  }*/
    tf
  }
}
