...
object NLPUtils {
  ...
  def createLabeledDocumentTest(wholeTextFile: (String,String), labelMap: Map[String, Int]
    ...
    // val (label, id) = getLabelandId(wholeTextFile._1)
    val processedDoc = tokenizeAndStem(wholeTextFile._1, stopWords)
...
//    }
  ...
  def tfidfTransformerTest(data: RDD[LabeledDocumentTest], norm: Boolean = false): RDD[Vector] = {
    val tf = new HashingTF()
    val freqs = data.map(x => tf.transform(x.body)).cache()
...
case class LabeledDocumentTest(body: Seq[String])
